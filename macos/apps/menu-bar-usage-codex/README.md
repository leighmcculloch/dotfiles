...
The displayed percentage is `100 - usedPercent` from the primary (5-hour) rate-limit window. Click the status item to see the 5-hour and weekly windows' remaining percentages and time left, as well as refresh, usage settings, launch-at-login, and quit. Hover over it to see each window's reset date and time remaining. A `—` means the installed client is unavailable, not signed in, offline, or returned no usable window.
