  ...
  return {
    event: async ({ event }) => {
      if (event.type === "session.idle" || event.type === "question.asked") {
        ...
        if (message) {
          await $`bash -c 'source ~/.zenv_apikey_pushover && curl -s -X POST https://api.pushover.net/1/messages.json -d token="$PUSHOVER_TOKEN" -d user="$PUSHOVER_USER" -d "message=${message}"'`
            .quiet().nothrow();
        }
      }
    },
